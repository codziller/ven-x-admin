...
const MobileBrandsOfTheMoment = () => {
  ...
  const forYouMobileBrandsOfTheMoment = mobileBrandsOfTheMoments?.filter(
    (_) => _?.isForYou
  );
          ...
          <>
            <div className="flex flex-col justify-start items-start gap-4 relative w-full">
              {/* <span className="text-black text-lg sm:text-xl font-700 my-5">
                Mobile Brands Of The Moment (For You)
              </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 smlg:grid-cols-4 gap-4 justify-between items-start w-full mb-5">
                {postArray?.map((item, i) => {
                  const slide = forYouMobileBrandsOfTheMoment?.[i];
                  return (
                    <Link
                      key={i + "card"}
                      to={
                        slide
                          ? `/dashboard/marketing/edit-mobile-brand-of-the-moment/${warehouse_id}/forYou/${slide?.id}`
                          : `/dashboard/marketing/add-mobile-brand-of-the-moment/${warehouse_id}/forYou`
                      }
